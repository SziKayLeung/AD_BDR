...
#SBATCH --output=4_log/4_pbalign_filter_batch2-%A_%a.o
#SBATCH --error=4_log/4_pbalign_filter_batch2-%A_%a.e
...
batch2_fa=($(find "${WKD_ROOT}/4_tclean/Batch2" -type f -name "*clean.fa")) 
fasta=${batch2_fa[${SLURM_ARRAY_TASK_ID}]}
sample=$(basename ${fasta} | cut -d "_" -f 1)
dir=${WKD_ROOT}/5_cupcake/5_align/Batch2/

...
### FACS 
Some of the high RIN AD samples were not selected for FACS...
...
As expected, higher RIN values correlated with lower PMI. All Braak Stage 6 samples (considered top 5 with highest RIN scores, threshold denoted with a dashed line, would have PMI < 50 hours - shaded green) bar 1 (shaded red). 3 out of 5 samples of Control have short PMI (shaded blue).  
**For AD samples, PMI vs RIN? Better to pick more closely matched PMI between AD samples and Control?**
```{r}
...
Final %>% 
  ...
  annotate("rect", xmin = 0, xmax = 50, ymin = 6.2, ymax = 8.5, alpha = .1,fill = "green") + 
  annotate("rect", xmin = 5, xmax = 25, ymin = 3.8, ymax = 6.5, alpha = .1,fill = "blue") +
  annotate("rect", xmin = 92, xmax = 100, ymin = 6, ymax = 6.4, alpha = .1,fill = "red") 
...
Final %>% 
  filter(BraakTangle_numeric %in% c("0","6")) %>% 
  datatable(., caption = "All Braak Stage 0 and 6",options = list(pageLength = 5, scrollX = TRUE)) %>% 
  formatStyle('BraakTangle_numeric',  backgroundColor = styleEqual(c(0, 6), c(wes_palette("Royal1")[1], wes_palette("Royal1")[2])))
```
...
# filter by Braak Stage 6 and order by RIN score
BS_0 <- Final %>% filter(BraakTangle_numeric == "0") 
BS_6 <- Final %>% filter(BraakTangle_numeric == "6") %>% filter(rank(desc(as.numeric(RINe))) <= 5)

Selected <- bind_rows(BS_0, BS_6)

datatable(Selected, caption = "All Braak Stage 0 and 6",options = list(pageLength = 5, scrollX = TRUE)) %>% 
  formatStyle('BraakTangle_numeric',  backgroundColor = styleEqual(c(0, 6), c(wes_palette("Royal1")[1], wes_palette("Royal1")[2])))

write.csv(Selected, paste0(input_dir,"Selected_BDR.csv"))
```

...
counts <- class %>% select(isoform,starts_with("FL."))
...
input_abundance <- read.table(paste0(input_dir,"/Post_IsoSeq/TOFU/AllBDRTargeted.Demultiplexed_Abundance.txt"),header=T, sep = ",")
colnames(input_abundance)
# Check the input abundance file is the same as the counts file
#counts %>% filter(isoform_id == "PB.9744.191")
#input_abundance %>% filter(id == "PB.9744.191")
...
abundance[2:length(abundance)] <- sapply(abundance[2:length(abundance)],as.numeric)
...
gtf <- rtracklayer::import(paste0(input_dir,"/Post_IsoSeq/SQANTI_TAMA_FILTER/AllBDRTargeted_sqantitamafiltered.classification.final.gtf"))
removed_genes <- c("PB.11271","PB.10907","PB.11275","PB.11277","PB.10908","PB.11278","PB.10905","PB.11283")
mod_counts <- counts %>% mutate(Gene = paste0("PB.",word(counts$isoform_id,c(2),sep = fixed("."))))
mod_counts <- mod_counts %>% filter(Gene %ni% removed_genes)
mod_counts <- mod_counts %>% select(-"Gene")

mod_abundance <- abundance %>% mutate(Gene = paste0("PB.",word(counts$isoform_id,c(2),sep = fixed("."))))
mod_abundance <- mod_abundance %>% filter(Gene %ni% removed_genes)
mod_abundance <- mod_abundance %>% select(-"Gene")
mod_abundance[2:length(mod_abundance)] <- sapply(mod_abundance[2:length(mod_abundance)],as.numeric)
...
aSwitchList <- importRdata(
  ...
  isoformExonAnnoation = paste0(input_dir,"/Post_IsoSeq/SQANTI_TAMA_FILTER/AllBDRTargeted_sqantitamafiltered.classification.final.gtf"),
  isoformNtFasta       = paste0(input_dir,"/Post_IsoSeq/SQANTI_TAMA_FILTER/AllBDRTargeted_sqantifiltered_tamafiltered_classification.fasta"),
)

...
countdata  <- class %>% select(transcript_name_id,starts_with("FL.")) %>% column_to_rownames(., var = "transcript_name_id")
...
# shrink 
resultsNames(dds)
resLFC <- lfcShrink(dds, coef="condition_AD_vs_Control", type="apeglm")
resLFC
